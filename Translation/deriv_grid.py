...
# позволяют построить ожидаемую кривую в соответствии с нашей моделью. Этот код
...
# plot_real, plot_abs постройте и сохраните указанные решение. 
...
class deriv_grid:
    ...
    def change_parameters(self, R=None, B=None, dk=None, mu=None, k=None, amp = None, grid_size = None, ang_lim = None):
        # Очистите данные для старых параметров.
        self.clear_run()

        # Измените параметры, заданные в коде.
        if R is not None: 
            self.R = R
        if B is not None:
            self.B = B
        ...
        # Примените это изменение к параметрам:
    ...
    def derivGrid(self, n = 200, pr=1.0, method = 'RK45', rtol = rtol, atol = atol, trim = 16, 
        ...
        # Места для наших решений (которые будут списками списков решений).
        '''
        u, l -- верхний, нижний
...
# ---------
    ...
    def plot_real(self, i, j, k, R, B, mu, no):
        ...
        # выходные данные в каждый момент времени, заданный нашим моделируемым
