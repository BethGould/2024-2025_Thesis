# eelib/bvp_rootfinder_functions.py
#
# Автор: Элизабет Гоулд
# Дата последнего изменения: 23.03.2026

# Здесь содержится мой дескриптор функции для решения краевой задачи с помощью поиска
# нулей функции, линейные модели, которые будут использоваться с этим дескриптором
# функции, и пример кода для выполнения поиска нулей функции.
...
#   -- Принимает параметры кольца mu, dk, B, R, A и k0 и вычисляет разницу между
#      заданной начальной производной psi dpsi0 = x[0] + 1.j*x[1] и требуемой
#      производной для соединения краевой задачи. Поиск нулей этой функции решает
#     краевую задачу.
#
...
#      и передайте yy в качестве дескриптора функции, для которой нужно найти нули.
#
#   -- Параметры кольца B и R указаны в реальных единицах, а не в процентах от их
#      максимального значения. 
#
#   -- Для этой функции требуется модель для k и M в зависимости от параметров.
#   -- k_calc_f(dpsi0, mu, dk, B, R, A, k0) and M_calc_f(dpsi0, mu, dk, B, R, A, k0)
#   -- Здесь значение dpsi0 представлено в виде единственного комплексного числа, а
#      значения B и R указаны в реальных единицах измерения, как они хранятся в
#      классе loop, а не в процентах от их максимального значения, как они
#      передаются в класс loop.


# find_root_both(dk, R, B, mu, method = 'broyden2', tol = 1e-20, ratio = 1.0,
#                check_solution = False, model_k=pred_fast_k_true,
#                model_M = pred_slow_k_v3)
#   -- Это функция, которая найдет одно из решений нелинейной краевой задачи, а также
#      решение линейной краевой задачи. Если значение check_solution равно false, то
#      оно вернет только решение нелинейной краевой задачи в виде массива из двух
#      действительных чисел с действительной и мнимой составляющими начальной
#      производной от psi.  Если значение check_solution равно true, то оно вернет 4
#      значения:
#          - Комплексное решение для начальной производной от psi для линейного случая.
#          - Значение функции, для которого нужно выяснить нуль, для этой (линейной)
#            производной.
#          - Комплексное решение для начальной производной от psi для нелинейного случая.
#          - Значение функции, для которого нужно выяснить нуль, для этой (нелинейной)
#            производной.
#   -- Входными данными являются:
#       - dk, R, B, mu, в реальных единицах, как получено из кольца.
...
# Существует код для якобиана, который сейчас не используется, так как он не был
# проверен и отлажен. Раньше были проблемы с соединением, поэтому мне нужно было
# переключить модель, и я не обновляла якобиан. Весь код для якобиана был скрыт
# от компьютера в виде комментариев.
...
# Они используют неиспользуемые параметры, поскольку их параметры должны
# соответствовать параметрам других моделей.
...
# Вот функция, для которой нужно найти нули.
# Обратите внимание, что это то же самое, что и в линейном случае, но со
# смещенными волновыми числами. Это приближение решения, а не фактическая
# ожидаемая форма, для которой быстрые колебания не являются полностью
# синусоидальными. Однако графики показывают, что это приемлемое
# приближение.
...
#       - method - Алгоритм поиска нуля из списка доступных алгоритмов SciPy.
#       - tol - Допуск для этого алгоритма.
#       - ratio - Первоначальное предположение соответствует линейному решению в виде
#                 dphi0 / ratio.
#       - model_k and model_M - это модели k and M для функции, для которой нужно найти
#         нули.
def find_root_both(dk, R, B, mu, method = 'broyden2', tol = 1e-20, ratio = 1.0,
    ...
    # Найти нуль линейного уравнения. Существует только один, поэтому метод и
    # первоначальное предположение не важны.
