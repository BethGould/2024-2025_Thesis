...
# входными данными, а mu_eff - это то, чему соответствуют решения.)
...
# скорее это просто результат неточности.
...
# Все эти переменные рассчитываются при установке параметров.
#   lngt     = 2 * pi * self.R
...
#   k_ee_estimated -- Оценили ли волновое число быстрых колебаний для
#         нелинейного случая на основе решения первых n колебаний?
#   k_0_estimated  -- Рассчитали ли волновое число быстрых колебаний для
#         линейного случая на основе решения первых n колебаний?
#   bvp_solved     -- Соответствует ли начальная производная psi той,
#         которая была определена путем решения краевой задачи?
#   deriv_set      -- Была ли изменена производная из решения линейной
#         краевой задачи?
...
#         позже изменится производную, но не параметры, решение будет сохранено.
...
class loop:
    ...
    # Удалить решения начальной задачи (поскольку они могут быть большими) и
    # указать, что ничего не было решено (поскольку любое предыдущее решение
    ...
    def update_params(self, R=-1., B=-1., dk=-1., mu=-1., k=-1., amp=-1.):
        ...
        self.deriv_set = False # False = производная по умолчанию,
                               # True = выбранная производная
        self.bvp_deriv = None  # Очистить старое решение.

        self.calcInit() # Пересчитать производные величины.
    ...
    def ivp_solver_steps(self, t0, tf, y0, yp0, n=None, ee_int=True, m = None, method = None, rtol = rtol, atol = atol, fullSol = False, estimate_k = False):
        ...
        while tfs <= tf:
            ...
            # Выбрать ту же сторону события действительной часть psi.
            ...
            # Изменить масштаб для нового запуска.
    ...
    # метода для решения ОДУ. self.solve_mu_0 можно изменить на значение
    # False, чтобы удалить оценку линейного решения и ускорить вычисления.
    # Нужно оценивать это, потому что T0 неверно оценивает период из-за ошибки. 
    ...
    # Это ток, предполагаемый для нелинейного решения.
    ...
    # Этот третий метод определяется в начальной точке (или в другой
