...
def test_match(dk, R2, B2, mu):
    ...
    # Выходные данные разделяют действительную и мнимую части производной
    # psi_0, как если бы они были отдельными переменными. Нужно объединить
    ...
    # Метод для решения ОДУ с решением краевой задаче для визуального
    # анализа согласия
    l_calc.solve_ivp(n = 1000, solve=1)
    solu = l_calc.solu

    # Весь приведенный ниже код является копией кода из кода для построения
    # производной сетки. Код извлекает x и psi из кольца (loop) для
    ...
    # Это точное решение, соответствующее огибающей быстрых колебаний.
